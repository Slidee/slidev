 ...
 * 
 * @class: symflaCoreEventsManager (symfla.core)
 * @file: symflaCoreEventsManager.as
 * @author: Enerol
...
package slidev.core
{
	...
	public class slidevCoreEventsManager 
	{
		protected var listenedTypes:Array;
		protected var dispatcherArray:Array;
		
		protected static var _Instance:slidevCoreEventsManager;
		protected static var _listenedQueue:Array;
		
		public function slidevCoreEventsManager(pMainObject:EventDispatcher)
		{
		...
		{ 
			_Instance = new slidevCoreEventsManager(pMainObject);
		...
		static public function addListenerFor(pObj:slidevCoreEvent):void
		{
			if(_listenedQueue == null){
				Dbg.t("You cannot add a listener while the Event Manager is not initted", "CoreEventsManager", Dbg.PRIORITY_FATAL);
				return;
			}
			if(!isIntoQueue(pObj)){
				...
				Dbg.t("Cannot register the symflaCoreEvent, it is already listened !", "CoreEventsManager", Dbg.PRIORITY_ERROR);
			}
		}
		...
		{
			if(_listenedQueue == null){
				Dbg.t("You cannot remove a listener while the Event Manager is not initted", "CoreEventsManager", Dbg.PRIORITY_FATAL);
				return false;
			}
			if(isIntoQueue(pObj)){
				...
				Dbg.t("Cannot delete the symflaCoreEvent, it is not listened !","CoreEventsManager", Dbg.PRIORITY_ERROR);
				return false;
			}
		}
		...
		static protected function sendEvent(pEvt:Event):void{
			Dbg.t("Event Catched :"+pEvt.type,"CoreEventsManager");
			var fCalled:int = 0
			for(var i:int =0; i< _listenedQueue.length;i++){
			...
			Dbg.t(fCalled+" Listening Objects Called","CoreEventsManager");			
		}
